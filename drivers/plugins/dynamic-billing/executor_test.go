...
func TestDynamicBilling_Executor(t *testing.T) {
	...
	plugin := &executor{
		...
		redisID:                 "mock_cache",
		balanceKeyGenerator:     NewKeyGenerator("balance:{application}"),
		priceKeyGenerator:       NewKeyGenerator("access-resource-price:{application}:{resource}"),
		taskKeyGenerator:        NewKeyGenerator("resource-pricing-task:{application}:{resource}"),
		concurrencyKeyGenerator: NewKeyGenerator("resource-pricing-concurrency:{application}:{resource}"),
	}
...
func TestDynamicBilling_ConcurrencyAndBalance(t *testing.T) {
	...
	plugin := &executor{
		...
		enableBalance:           true,
		balanceKeyGenerator:     NewKeyGenerator("balance:{user}"),
		priceKeyGenerator:       NewKeyGenerator("access-resource-price:{user}:{resource}"),
		taskKeyGenerator:        NewKeyGenerator("resource-pricing-task:{user}:{resource}"),
		concurrencyKeyGenerator: NewKeyGenerator("resource-pricing-concurrency:{user}:{resource}"),
	}
...
func TestDynamicBilling_TaskPricingSnapshot(t *testing.T) {
	...
	plugin := &executor{
		...
		balanceKeyGenerator:     NewKeyGenerator("balance:{user}"),
		priceKeyGenerator:       NewKeyGenerator("access-resource-price:{user}:{resource}"),
		taskKeyGenerator:        NewKeyGenerator("dynamic-billing-task:{user}:{resource}"),
		concurrencyKeyGenerator: NewKeyGenerator("dynamic-billing-concurrency:{user}:{resource}"),
	}
