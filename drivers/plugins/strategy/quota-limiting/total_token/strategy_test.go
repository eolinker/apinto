...
func TestMockCachePreDeductAndRollback(t *testing.T) {
	...
	// 1. 预扣 1000
	mc.IncrBy(ctx, "k1", 1000, 0)
	if mc.data["k1"] != 1000 {
		t.Fatalf("expected 1000, got %d", mc.data["k1"])
	}

	// 2. 超额回滚 1000
	mc.DecrBy(ctx, "k1", 1000, 0)
	if mc.data["k1"] != 0 {
		t.Fatalf("expected 0, got %d", mc.data["k1"])
	}

	// 3. 补扣（差额为 +200）
	mc.IncrBy(ctx, "k1", 1000, 0) // 先预扣 1000
	mc.IncrBy(ctx, "k1", 200, 0)  // 实际 1200，补扣 200
	if mc.data["k1"] != 1200 {
		t.Fatalf("expected 1200, got %d", mc.data["k1"])
	}
}
