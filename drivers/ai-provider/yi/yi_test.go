...
// TestSentTo tests the end-to-end execution of the yi integration.
...
func runTest(apiKey string, requestBody []byte, wantStatus string, validFunc func(ctx eocontext.EoContext) bool) error {
	...
	// Create the worker
	worker, err := Create("yi", "yi", cfg, nil)
	if err != nil {
	...
	err = executeConverter(ctx, handler, "yi-large", "https://api.lingyiwanwu.com")
	if err != nil {
