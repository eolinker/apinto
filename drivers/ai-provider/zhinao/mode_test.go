...
func TestSentTo(t *testing.T) {
	...
	testData := []struct {
		...
		{
			name:       "success",
			apiKey:     os.Getenv("ZHINAO_VALID_API_KEY"),
		...
		{
			name:       "invalid key",
			apiKey:     os.Getenv("ZHINAO_INVALID_API_KEY"),
			wantStatus: ai_provider.StatusInvalid,
		},
		{
			name:       "expired key",
			apiKey:     os.Getenv("ZHINAO_EXPIRE_API_KEY"),
			wantStatus: ai_provider.StatusInvalid,
		},
	}
...
func runTest(apiKey string, requestBody []byte, wantStatus string) error {
	...
	baseDomain := "https://api.360.cn"
	// Create the worker
	worker, err := Create("zhinao", "zhinao", cfg, nil)
	if err != nil {
	...
	err = executeConverter(ctx, handler, "360gpt2-pro", baseDomain)
	if err != nil {
