...
// TestSentTo tests the end-to-end execution of the deepseek integration.
...
func runTest(apiKey string, requestBody []byte, wantStatus string, validFunc func(ctx eocontext.EoContext) bool) error {
	...
	// Create the worker
	worker, err := Create("deepseek", "deepseek", cfg, nil)
	if err != nil {
	...
	err = executeConverter(ctx, handler, "deepseek-chat", "https://api.deepseek.com")
	if err != nil {
