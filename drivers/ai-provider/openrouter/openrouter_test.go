...
// TestSentTo tests the end-to-end execution of the openrouter integration.
...
func runTest(apiKey string, requestBody []byte, wantStatus string, validFunc func(ctx eocontext.EoContext) bool) error {
	...
	// Create the worker
	worker, err := Create("openrouter", "openrouter", cfg, nil)
	if err != nil {
	...
	err = executeConverter(ctx, handler, "openai/gpt-3.5-turbo", "https://openrouter.ai")
	if err != nil {
