...
func (c *Chat) ResponseConvert(ctx eocontext.EoContext) error {
	...
	case 200:
		...
		convert.SetAIModelInputToken(ctx, usage.InputTokens)
		convert.SetAIModelOutputToken(ctx, usage.OutputTokens)
		convert.SetAIModelTotalToken(ctx, usage.InputTokens+usage.OutputTokens)
	case 400:
