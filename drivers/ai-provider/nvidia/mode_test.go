...
func TestSentTo(t *testing.T) {
	...
	testData := []struct {
		...
		{
			name:       "success",
			apiKey:     os.Getenv("NVIDIA_VALID_API_KEY"),
		...
		{
			name:       "invalid key",
			apiKey:     os.Getenv("NVIDIA_INVALID_API_KEY"),
			wantStatus: ai_provider.StatusInvalidRequest,
		},
		{
			name:       "expired key",
			apiKey:     os.Getenv("NVIDIA_EXPIRE_API_KEY"),
			wantStatus: ai_provider.StatusInvalidRequest,
		},
	}
...
func runTest(apiKey string, requestBody []byte, wantStatus string) error {
	...
	baseDomain := "https://integrate.api.nvidia.com"
	// Create the worker
	worker, err := Create("nvidia", "nvidia", cfg, nil)
	if err != nil {
	...
	err = executeConverter(ctx, handler, "meta/llama-3.1-405b-instruct", baseDomain)
	if err != nil {
