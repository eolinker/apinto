...
func init() {
	driverCreate.Set(ai_convert.ModelTypeOpenAIChat, func(mt ai_convert.ModelType, conf *Config) (ai_convert.IConverterDriver, error) {
		handler, err := ai_convert.NewOpenAIChat(provider, conf.APIKey, conf.BaseUrl, mt, 0, nil, errorCallback)
		if err != nil {
			return nil, err
		}
		return &Chat{
