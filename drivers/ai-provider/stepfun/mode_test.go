...
func TestSentTo(t *testing.T) {
	...
	testData := []struct {
		...
		{
			name:       "success",
			apiKey:     os.Getenv("STEP_FUN_VALID_API_KEY"),
...
func runTest(apiKey string, requestBody []byte, wantStatus string) error {
	...
	baseDomain := "https://api.stepfun.com"
	// Create the worker
	worker, err := Create("stepfun", "stepfun", cfg, nil)
	if err != nil {
	...
	err = executeConverter(ctx, handler, "step-1-8k", baseDomain)
	if err != nil {
