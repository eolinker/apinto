...
// TestSentTo tests the end-to-end execution of the tongyi integration.
...
func runTest(apiKey string, requestBody []byte, wantStatus string, validFunc func(ctx eocontext.EoContext) bool) error {
	...
	// Create the worker
	worker, err := Create("tongyi", "tongyi", cfg, nil)
	if err != nil {
	...
	err = executeConverter(ctx, handler, "qwen-plus", "https://dashscope.aliyuncs.com")
	if err != nil {
